...
export const Route = createFileRoute("/colleges")({
  head: () => ({
    meta: [
      { title: "College Uniforms — Alkausar Uniforms" },
      {
        name: "description",
        content:
          "Find your college's uniform in a few clicks. Pick college, campus, collection and academic level.",
      },
      { property: "og:title", content: "College Uniforms — Alkausar Uniforms" },
      {
        property: "og:description",
        content:
          "Guided college uniform finder — college, campus, collection, academic level.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: CollegeUniforms,
});

/* ---------- Persistence ---------- */

const STORAGE_KEY = "alkausar:college-finder:v2";
...
function CollegeUniforms() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    saveState({ collegeId, campusId, collection, classId, applied });
  }, [hydrated, collegeId, campusId, collection, classId, applied]);

  const collegesQ = useQuery({
    queryKey: ["storefront", "colleges"],
    queryFn: () => collegesFn(),
    staleTime: 5 * 60_000,
  });

  const campusesQ = useQuery({
    queryKey: ["storefront", "college_campuses", collegeId],
    queryFn: () => campusesFn({ data: { collegeId: collegeId! } }),
    enabled: !!collegeId,
  });

  const classesQ = useQuery({
    queryKey: ["storefront", "college_classes", collegeId],
    queryFn: () => classesFn({ data: { collegeId: collegeId! } }),
    enabled: !!collegeId,
  });
  ...
  const canSearch =
    !!collegeId && !!collection && !!classId && (!campusRequired || !!campusId);
  ...
  const steps = useMemo(() => {
    ...
    if (collegeId && campusLoaded && hasCampuses) {
      arr.push({
        key: "campus",
        n: n++,
    ...
    if (collegeId && (!campusRequired || campusId)) {
      arr.push({
        key: "collection",
        n: n++,
    ...
    if (collegeId && collection && (!campusRequired || campusId)) {
      arr.push({
        key: "class",
        n: n++,
  ...
  const filteredColleges = useMemo(() => {
    const list = collegesQ.data ?? [];
    const q = collegeQuery.trim().toLowerCase();
    if (!q) return list;
    return list.filter((s) => s.name.toLowerCase().includes(q));
  }, [collegesQ.data, collegeQuery]);
              ...
              <StepBlock n={s.n} title={s.title} focused={focusedKey === s.key}>
                {s.key === "college" ? (
                  <CollegeChooser
                    ...
                    totalCount={(collegesQ.data ?? []).length}
                    selectedId={collegeId}
                    onSelect={onCollegeChange}
                    query={collegeQuery}
                    onQueryChange={setCollegeQuery}
                  />
                ) : (
                  s.node
                )}
          ...
          >
            ...
            >
              <div className="w-full sm:flex-1 min-w-0 flex flex-wrap items-stretch gap-2">
                ...
                />
                <Chip label="Level" value={selectedClass?.name ?? null} />
              </div>
  ...
  return (
    <div
      className={[
        ...
        focused ? "border-[#CF0A0A] ring-4 ring-[#CF0A0A]/15" : "border-black/5",
      ].join(" ")}
    >
...
function CollegeChooser({
  ...
  if (totalCount === 0)
    return <InlineEmpty icon={CollegeIcon} text="No colleges available yet." />;
  return (
    <div className="space-y-5">
      ...
      {colleges.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-black/10 bg-white p-8 text-center">
          <CollegeIcon className="h-7 w-7 text-black/20 mx-auto mb-2" />
              ...
              >
                ...
                <div className="w-full aspect-square bg-white grid place-items-center p-5">
                  {s.logoUrl ? (
                    ...
                    <CollegeIcon className="h-10 w-10 text-black/20" />
                  )}
                </div>
...
function ClassChooser({
  ...
  if (classes.length === 0)
    return <InlineEmpty icon={Tag} text="No academic levels configured for this college." />;
  return (
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {college && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-black text-white text-[11px] font-medium px-3 py-1">
              <CollegeIcon className="h-3 w-3" /> {college.name}
            </span>
          )}
          {campusLabel && (
    ...
    <article className="group relative rounded-3xl bg-white border border-black/5 overflow-hidden hover:-translate-y-1 hover:shadow-[0_28px_60px_-28px_rgba(0,0,0,0.28)] transition-all duration-300 flex flex-col">
      ...
      >
        ...
        <div className="absolute top-3 right-3 z-10">
          <WishlistButton module="college" productId={product.id} size="sm" />
        </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <Link to="/product/college/$id" params={{ id: product.id }} className="block group/name">
  ...
  return (
    <div className="rounded-3xl border border-dashed border-black/10 bg-white p-12 text-center">
      ...
      <p className="mt-2 text-sm text-black/50 max-w-md mx-auto leading-relaxed">
        Try a different academic level, campus or collection.
      </p>
    </div>
  );
}
