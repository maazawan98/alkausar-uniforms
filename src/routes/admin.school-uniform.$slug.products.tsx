...
export const Route = createFileRoute("/admin/school-uniform/$slug/products")({
  loader: async ({ params }) => {
  ...
  notFoundComponent: () => (
    <div className="text-center py-20 text-sm text-black/50">School not found</div>
  ),
...
function AllProductsView() {
  ...
  const listFn = useServerFn(listSchoolProducts);
  const exportFn = useServerFn(getProductsExport);
  const [exporting, setExporting] = useState(false);
  const dataQ = useQuery({
    queryKey: ["school-products", school.id],
    queryFn: () => listFn({ data: { schoolId: school.id } }),
  });

  const handleExport = async () => {
    setExporting(true);
    try {
      const payload = await exportFn({ data: { schoolId: school.id } });
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 mb-5">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-[#6B7280]">
              Browse every product created for this school. This page is read-only.
            </p>
          </div>
        ...
        {dataQ.isLoading ? (
          ...
          <div className="text-center py-20 border-2 border-dashed border-[#E5E7EB] rounded-2xl bg-white">
            ...
            <p className="text-sm text-[#6B7280] mt-1">
              This school doesn't have any products yet.
            </p>
          </div>
        ) : filtered.length === 0 ? (
...
function ProductRow({
  ...
  const customerPreview = `${schoolName} ${collLabel} ${p.name} ${p.category_name}`.trim();
  const priceLabel =
    p.priceFrom == null
      ? "—"
...
function ProductDetailsDrawer({
  ...
  const p = q.data;
  const collLabel = p?.collection_type === "boys" ? "Boys" : "Girls";
  const customerName = p ? `${schoolName} ${collLabel} ${p.name}` : "";
