...
import { CollegeCategoryDialog as CategoryDialog } from "@/components/admin/CollegeCategoryDialog";
import {
  getCollege,
  listCategories,
  deleteCategory,
...
function CategoriesPage() {
  ...
  const [editing, setEditing] = useState<CollegeCategory | null>(null);
  const [deleting, setDeleting] = useState<CollegeCategory | null>(null);

  const query = useQuery({
    queryKey: ["categories", college?.id, coll],
    queryFn: () => list({ data: { collegeId: college!.id, collection: coll } }),
    enabled: !!college,
  });
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["categories", college?.id, coll] });
      ...
      <div className="flex items-start justify-between gap-4 pb-6 mb-8 border-b border-[#E5E7EB]">
        <div>
          ...
          <p className="mt-2 text-[15px] text-black/55">
            Manage categories for this collection. Each category is isolated per college.
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      {query.isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-6 gap-4">
          {query.data!.map((c) => (
            ...
            >
              <Link
                to="/admin/colleges/$slug/$collection/$categoryId"
                params={{ slug: college.slug, collection: coll, categoryId: c.id }}
                className="block"
              >
      ...
      <AlertDialog open={!!deleting} onOpenChange={(v) => !v && setDeleting(null)}>
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This category will be removed from {college.name}. Any linked products must be removed first.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
