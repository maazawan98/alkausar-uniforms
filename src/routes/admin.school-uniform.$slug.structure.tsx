...
export const Route = createFileRoute("/admin/school-uniform/$slug/structure")({
  loader: async ({ params }) => {
    ...
    <div>
      ...
      <div className="pb-6 mb-8 border-b border-[#E5E7EB]">
        ...
        <p className="mt-2 text-[15px] text-black/55">
          Manage the classes and campuses for this school.
        </p>
      </div>
      <div className="space-y-8">
        <ClassesModule schoolId={school.id} />
        <CampusesModule schoolId={school.id} />
      </div>
    </div>
  );
}
...
function ClassesModule({ schoolId }: { schoolId: string }) {
  ...
  const [order, setOrder] = useState<SchoolClass[]>([]);

  const q = useQuery({
    queryKey: ["classes", schoolId],
    queryFn: () => list({ data: { schoolId } }),
  });
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["classes", schoolId] });

  const save = useMutation({
    mutationFn: (name: string) =>
      upsert({ data: { id: editing?.id, schoolId, name } }),
    onSuccess: () => {
  ...
  const standard = useMutation({
    mutationFn: () => addStandard({ data: { schoolId } }),
    onSuccess: (res) => {
  ...
  const reorderMut = useMutation({
    mutationFn: (ids: string[]) => reorder({ data: { schoolId, ids } }),
    onSuccess: () => {
...
function CampusesModule({ schoolId }: { schoolId: string }) {
  const qc = useQueryClient();
  ...
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<SchoolCampus | null>(null);
  const [deleting, setDeleting] = useState<SchoolCampus | null>(null);
  const [order, setOrder] = useState<SchoolCampus[]>([]);
  ...
  const reorderMut = useMutation({
    mutationFn: (ids: string[]) => reorder({ data: { schoolId, ids } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["campuses", schoolId] }),
    onError: (e: Error) => {
      toast.error(e.message);
      qc.invalidateQueries({ queryKey: ["campuses", schoolId] });
    },
...
function CampusDialog({
  ...
  onOpenChange: (v: boolean) => void;
  schoolId: string;
  editing: SchoolCampus | null;
  existing: SchoolCampus[];
}) {
  const qc = useQueryClient();
