...
import { getCategory, getCollege, listCampuses, listClasses } from "@/lib/college.functions";
import {
  deleteProduct,
  getProduct,
  listProducts,
...
import { CollegeProductDialog as ProductDialog } from "@/components/admin/CollegeProductDialog";

export const Route = createFileRoute("/admin/colleges/$slug/$collection/$categoryId")({
  loader: async ({ params }) => {
    const [category, college] = await Promise.all([
      getCategory({ data: { id: params.categoryId } }),
      getCollege({ data: { slug: params.slug } }),
    ]);
    if (!category || !college) throw notFound();
...
function CategoryDetail() {
  const { category, college } = Route.useLoaderData();
  ...
  const handleExport = async () => {
    ...
    try {
      const payload = await exportFn({
        data: { collegeId: category.college_id, categoryId },
      });
  ...
  const productsQ = useQuery<ProductRow[]>({
    queryKey: ["college_products", categoryId],
    queryFn: () => listFn({ data: { categoryId } }),
  });
  const campusesQ = useQuery({
    queryKey: ["campuses", category.college_id],
    queryFn: () => campusFn({ data: { collegeId: category.college_id } }),
  });
  const classesQ = useQuery({
    queryKey: ["classes", category.college_id],
    queryFn: () => classesFn({ data: { collegeId: category.college_id } }),
  });
  ...
  return (
    <div>
      <Link
        to="/admin/colleges/$slug/$collection"
...
function ProductTableRow({
  ...
  const customerPreview = `${collegeName} ${collLabel} ${p.name} ${categoryName}`.trim();
