...
function CollegeDetail() {
  ...
  const boysQ = useQuery({
    queryKey: ["categories", college?.id, "boys"],
    queryFn: () => catsFn({ data: { collegeId: college!.id, collection: "boys" } }),
    enabled: !!college,
  });
  const girlsQ = useQuery({
    queryKey: ["categories", college?.id, "girls"],
    queryFn: () => catsFn({ data: { collegeId: college!.id, collection: "girls" } }),
    enabled: !!college,
  });
  const classesQ = useQuery({
    queryKey: ["classes", college?.id],
    queryFn: () => classesFn({ data: { collegeId: college!.id } }),
    enabled: !!college,
  });
  const campusesQ = useQuery({
    queryKey: ["campuses", college?.id],
    queryFn: () => campusesFn({ data: { collegeId: college!.id } }),
    enabled: !!college,
  });
  ...
  const modules = [
    {
      to: "/admin/colleges/$slug/products",
      title: "View All Products",
      description: "Browse every product created for this college.",
      icon: ShoppingBag,
    ...
    {
      to: "/admin/colleges/$slug/structure",
  ...
  return (
    <div className="mx-auto max-w-[1600px] w-[calc(100%-48px)] my-6">
      ...
      <nav className="flex items-center gap-2 text-[13px] text-[#6B7280] mb-4">
        ...
        <span className="text-black font-medium truncate">{college.name}</span>
      </nav>
          ...
          <div className="flex min-w-0 items-start gap-5">
            <div className="h-[120px] w-[120px] shrink-0 rounded-2xl bg-white border border-[#E5E7EB] grid place-items-center overflow-hidden shadow-[0_1px_2px_rgba(16,24,40,0.04),0_4px_12px_-6px_rgba(16,24,40,0.08)]">
              {college.logoUrl ? (
                ...
                <CollegeIcon className="h-10 w-10 text-black/25" />
              )}
            </div>
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] text-[#6B7280] font-mono">/{college.slug}</p>
                ...
                >
                  <span
                    className={[
                      ...
                      college.is_active ? "bg-[#22C55E]" : "bg-black/30",
                    ].join(" ")}
                  />
        ...
        <div className="flex items-end justify-between mb-4">
          <div>
            ...
            <p className="text-[13px] text-[#6B7280] mt-0.5">
              Manage products, collections, and structure for this college.
            </p>
          </div>
        </div>
          ...
          {modules.map((m) => {
            ...
            return (
              <Link
                key={m.title}
                to={m.to as any}
                params={{ slug: college.slug } as any}
                className={[
