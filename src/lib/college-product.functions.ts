...
async function assertCampusesInCollege(collegeId: string, ids: string[]) {
...
async function assertCategoryMatches(
  ...
  const { data, error } = await db
    ...
    .select("id, college_id, collection_type")
    .eq("id", categoryId)
    .maybeSingle();
  if (error) throw new Error(error.message);
  if (!data || data.college_id !== collegeId || data.collection_type !== collection) {
    throw new Error("Category does not belong to this college/collection");
  }
}

async function assertClassesInCollege(collegeId: string, ids: string[]) {
  if (!ids.length) return;
  const db = await getDb();
  const { data, error } = await db
...
async function insertClassMappings(
  ...
  await assertClassesInCollege(collegeId, allClassIds);

  const rows: { product_id: string; product_size_id: string; college_class_id: string }[] = [];
  const seen = new Set<string>();
  for (const m of mappings) {
    ...
    for (const cid of m.class_ids) {
      ...
      seen.add(key);
      rows.push({ product_id: productId, product_size_id: sid, college_class_id: cid });
    }
  }
  if (!rows.length) return;
  const { error } = await db.from("college_product_classes").insert(rows);
  if (error) throw new Error(error.message);
}
...
export const listProducts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<ProductRow[]> => {
    ...
    const [{ data: imgs }, { data: szs }] = await Promise.all([
      db.from("college_product_images").select("product_id, image, is_primary, sort_order").in("product_id", ids),
      db.from("college_product_sizes").select("product_id, price").in("product_id", ids),
    ]);
...
export const getProduct = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<ProductDetail | null> => {
    ...
    const [images, sizes, colours, tags, camps, pcls] = await Promise.all([
      db.from("college_product_images").select("id, image, is_primary, sort_order").eq("product_id", data.id).order("sort_order"),
      db.from("college_product_sizes").select("id, size, price, sale_price, sort_order").eq("product_id", data.id).order("sort_order"),
      db.from("college_product_colours").select("id, colour_name, hex_code, sort_order").eq("product_id", data.id).order("sort_order"),
      db.from("college_product_quality_tags").select("tag").eq("product_id", data.id),
      db.from("college_product_campuses").select("campus_id").eq("product_id", data.id),
      db.from("college_product_classes").select("product_size_id, college_class_id").eq("product_id", data.id),
    ]);

    const productTypeNames = await fetchProductTypes(db, "college", data.id);
    ...
    for (const r of (pcls.data ?? []) as { product_size_id: string; college_class_id: string }[]) {
      const arr = classesBySizeId.get(r.product_size_id) ?? [];
      arr.push(r.college_class_id);
...
export const createProduct = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await assertCategoryMatches(data.category_id, data.college_id, data.collection_type);
    await assertCampusesInCollege(data.college_id, data.campus_ids);
    ...
    try {
      ...
      if (preparedImages.length) {
        const { error: imgErr } = await db.from("college_product_images").insert(
      ...
      if (data.campus_ids.length) {
        const { error: cErr } = await db.from("college_product_campuses").insert(
          data.campus_ids.map((cid) => ({ product_id: productId, campus_id: cid })),
        );
        if (cErr) throw new Error(cErr.message);
      }

      await replaceProductTypes(db, "college", productId, data.product_types);

      const tagsUnique = Array.from(new Set(data.quality_tags.map(normalizeTag))).filter(Boolean);
      if (tagsUnique.length) {
        const { error: tErr } = await db.from("college_product_quality_tags").insert(
      ...
      if (data.colours.length) {
        const { error: colErr } = await db.from("college_product_colours").insert(
...
export const updateProduct = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (toDelete.length) {
      await db.from("college_product_images").delete().in("id", toDelete.map((i) => i.id));
    ...
    await db.from("college_product_images").update({ is_primary: false }).eq("product_id", data.id);
    ...
    try {
      for (let i = 0; i < data.images.length; i++) {
        ...
        if (img.id) {
          ...
          const key = await uploadImage(img.upload);
          uploadedKeys.push(key);
          const { error } = await db.from("college_product_images").insert({
            product_id: data.id,
            image: key,
    ...
    // Sizes: wipe & reinsert. college_product_classes cascades on size delete, so it clears too.
    await db.from("college_product_sizes").delete().eq("product_id", data.id);
    const { data: insertedSizes, error: sErr } = await db
      .from("college_product_sizes")
      .insert(
        data.sizes.map((s, i) => ({
    ...
    await db.from("college_product_campuses").delete().eq("product_id", data.id);
    if (data.campus_ids.length) {
      const { error: cErr } = await db.from("college_product_campuses").insert(
    ...
    await db.from("college_product_quality_tags").delete().eq("product_id", data.id);
    await replaceProductTypes(db, "college", data.id, data.product_types);
    ...
    await db.from("college_product_colours").delete().eq("product_id", data.id);
    if (data.colours.length) {
      const { error: colErr } = await db.from("college_product_colours").insert(
        data.colours.map((c, i) => ({
          product_id: data.id,
...
export const deleteProduct = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await db
      ...
      .eq("module", "college")
      .eq("product_id", data.id);
    const { error } = await db.from("college_products").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
...
// ------- LIST ALL PRODUCTS BY COLLEGE (READ-ONLY OVERVIEW) -------
export type CollegeProductRow = {
  id: string;
  name: string;
  collection_type: "boys" | "girls";
  category_id: string;
...
export type CollegeProductsPayload = {
  rows: CollegeProductRow[];
  categories: { id: string; name: string; collection_type: "boys" | "girls" }[];
};

export const listCollegeProducts = createServerFn({ method: "GET" })
  .inputValidator((raw: unknown) =>
    z.object({ collegeId: z.string().uuid() }).parse(raw),
  )
  .handler(async ({ data }): Promise<CollegeProductsPayload> => {
    await ensureAdmin();
    const db = await getDb();
    ...
    const [{ data: products, error: pErr }, { data: cats }, { data: campuses }, { data: classes }] =
      await Promise.all([
        ...
        db
          ...
          .eq("college_id", data.collegeId),
        db.from("college_campuses").select("id, campus_name, area, city").eq("college_id", data.collegeId),
        db.from("college_classes").select("id, name").eq("college_id", data.collegeId).order("sort_order"),
      ]);
    ...
    const [imgs, szs, camps, pcls] = await Promise.all([
      db.from("college_product_images").select("product_id, image, is_primary, sort_order").in("product_id", ids),
      db.from("college_product_sizes").select("product_id, size, price, sort_order").in("product_id", ids).order("sort_order"),
    ...
    for (const r of pcls.data ?? []) {
      const name = classById.get(r.college_class_id);
      if (!name) continue;
    ...
    const rows: CollegeProductRow[] = await Promise.all(
