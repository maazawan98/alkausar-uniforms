...
async function decorateCollege(s: {
  id: string;
  name: string;
  slug: string;
  logo: string | null;
  is_active: boolean;
  created_at: string;
}, categoryCount: number): Promise<CollegeRow> {
  return {
    ...s,
...
export const listColleges = createServerFn({ method: "GET" }).handler(async (): Promise<CollegeRow[]> => {
  await ensureAdmin();
  const db = await getDb();
  ...
  return Promise.all(rows.map((r, i) => decorateCollege(r, counts[i])));
});

export const getCollege = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<CollegeRow | null> => {
    ...
    const { count } = await db
      .from("college_categories")
      .select("id", { count: "exact", head: true })
      .eq("college_id", row.id);
    return decorateCollege(row, count ?? 0);
  });
...
export const createCollege = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const logo = data.upload ? await uploadAsset("colleges", data.upload) : null;
    const { data: row, error } = await db
      .from("colleges")
      .insert({
        name: data.name,
    ...
    if (error) {
      ...
      if (error.code === "23505") {
        throw new Error("A college with this name or slug already exists.");
      }
...
export const updateCollege = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.upload) newLogoKey = await uploadAsset("colleges", data.upload);
...
export const deleteCollege = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if ((count ?? 0) > 0) {
      throw new Error(
        "This college still has categories. Remove all categories (and their products) before deleting the college.",
      );
    }
    const { data: existing } = await db
      ...
      .eq("id", data.id)
      .maybeSingle();
    const { error } = await db.from("colleges").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
...
export type CollegeClass = { id: string; name: string; college_id: string; sort_order: number };

export const STANDARD_LEVEL_NAMES = [
  "1st Year",
  "2nd Year",
  "3rd Year",
  "4th Year",
...
async function nextClassSortOrder(collegeId: string): Promise<number> {
  const db = await getDb();
  const { data } = await db
...
export const listClasses = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<CollegeClass[]> => {
    await ensureAdmin();
    const db = await getDb();
    const { data: rows, error } = await db
      .from("college_classes")
      .select("id, name, college_id, sort_order")
...
export const upsertClass = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.id) {
      ...
      if (error) {
        if (error.code === "23505")
          throw new Error("A class with this name already exists for this college.");
        throw new Error(error.message);
      }
    } else {
      const sort_order = await nextClassSortOrder(data.collegeId);
      const { error } = await db
        .from("college_classes")
        .insert({ college_id: data.collegeId, name: data.name, sort_order });
      if (error) {
        if (error.code === "23505")
          throw new Error("A class with this name already exists for this college.");
        throw new Error(error.message);
...
export const addStandardClasses = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const start = await nextClassSortOrder(data.collegeId);
    const toInsert = STANDARD_LEVEL_NAMES
      .filter((n) => !taken.has(n.toLowerCase()))
      .map((n, i) => ({ college_id: data.collegeId, name: n, sort_order: start + i }));
    if (toInsert.length === 0) return { inserted: 0 };
    const { error } = await db.from("college_classes").insert(toInsert);
    if (error) throw new Error(error.message);
...
export const deleteClasses = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await ensureAdmin();
    const db = await getDb();
    const { error } = await db.from("college_classes").delete().in("id", data.ids);
    if (error) throw new Error(error.message);
...
async function nextCampusSortOrder(collegeId: string): Promise<number> {
  const db = await getDb();
  const { data } = await db
...
export const listCampuses = createServerFn({ method: "GET" })
  .inputValidator((raw: unknown) => z.object({ collegeId: z.string().uuid() }).parse(raw))
  .handler(async ({ data }): Promise<CollegeCampus[]> => {
    await ensureAdmin();
    const db = await getDb();
    const { data: rows, error } = await db
      .from("college_campuses")
      .select("id, college_id, country, city, area, campus_name, sort_order")
      .eq("college_id", data.collegeId)
      .order("sort_order", { ascending: true });
    if (error) throw new Error(error.message);
    return (rows ?? []) as CollegeCampus[];
  });
...
export const upsertCampus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Detect other campuses with same country+city+area in this college
    ...
    if (data.id) {
      ...
      if (error) {
        if (error.code === "23505")
          throw new Error(
            "A campus with the same Country, City, Area and Name already exists for this college.",
          );
        throw new Error(error.message);
      }
    } else {
      const sort_order = await nextCampusSortOrder(data.collegeId);
      const { error } = await db.from("college_campuses").insert({
...
export const deleteCampus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await ensureAdmin();
    const db = await getDb();
    const { error } = await db.from("college_campuses").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const listCategories = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<CollegeCategory[]> => {
    await ensureAdmin();
    const db = await getDb();
    const { data: rows, error } = await db
      .from("college_categories")
      .select("id, college_id, collection_type, name, image, show_on_homepage")
      .eq("college_id", data.collegeId)
...
export const getCategory = createServerFn({ method: "GET" })
  .inputValidator((raw: unknown) => z.object({ id: z.string().uuid() }).parse(raw))
  .handler(async ({ data }): Promise<CollegeCategory | null> => {
    await ensureAdmin();
    const db = await getDb();
    const { data: row, error } = await db
      .from("college_categories")
...
export const createCategories = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await db.from("college_categories").insert(toInsert);
    if (error) throw new Error(error.message);
...
export const updateCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await db.from("college_categories").update(patch).eq("id", data.id);
    if (error) {
...
export const deleteCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: existing } = await db
      ...
      .eq("id", data.id)
      .maybeSingle();
    const { error } = await db.from("college_categories").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
