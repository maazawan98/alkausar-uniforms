...
import type { ExportPayload, ExportProductRow } from "./medical-product-export.functions";
...
const COL_WIDTHS = [30, 10, 10, 10, 12, 9, 20, 22, 26, 26, 24, 20, 50, 28, 14, 14];

export async function downloadMedicalProductsExport(payload: ExportPayload): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = "Alkausar Uniforms";
  wb.created = new Date();
  const ws = wb.addWorksheet("Medical Products");

  const titleRow = ws.addRow(["Alkausar Uniforms — Medical Product Export"]);
  ...
  a.href = url;
  a.download = "Medical_Products.xlsx";
