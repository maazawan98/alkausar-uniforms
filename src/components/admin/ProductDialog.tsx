...
import type { SchoolCampus, SchoolClass } from "@/lib/school-uniform.functions";
...
export function ProductDialog({
  ...
  // Class mappings keyed by size draft.key -> Set<schoolClassId>
  ...
  const customerPreview = useMemo(() => {
    ...
    return parts.join(" ");
  }, [schoolName, collectionType, name, categoryName]);
      ...
      <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto rounded-2xl">
        ...
        <Section title="Classes (Optional)">
          {classes.length === 0 ? (
            <p className="text-xs text-black/50">
              No classes configured for this school. Add classes in the School Structure section.
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Section title="Available at Campuses">
            {campuses.length === 0 ? (
              <p className="text-xs text-black/50">No campuses configured for this school.</p>
            ) : (
