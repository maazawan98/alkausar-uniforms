...
import type { CollegeCampus, CollegeClass } from "@/lib/college.functions";
...
function ImageTile({
  ...
  return (
    ...
    >
      <div {...attributes} {...listeners} className="absolute top-1 left-1 z-10 cursor-grab active:cursor-grabbing rounded-md bg-black/50 text-white p-1">
...
function TemplatePicker({
  ...
  const handleOk = () => {
    ...
    // Skip templates whose size already exists in the product
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col rounded-2xl">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto space-y-4 pr-1">
            {q.isLoading ? (
              ...
              grouped.map((group) => (
                <div key={group.label} className="rounded-xl border border-[#E5E7EB] overflow-hidden">
                    ...
                    {group.items.map((t) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {t.size}
                            <span className="ml-2 text-xs text-black/45">· {t.measurement_unit}</span>
                            {dup && (
                              <span className="ml-2 text-[11px] text-black/45">already added</span>
                            )}
                          </button>
                          <button
                            type="button"
...
// ---------------- Main Dialog ----------------
export function CollegeProductDialog({
  open,
  onOpenChange,
  collegeId,
  collegeName,
  ...
  // Class mappings keyed by size draft.key -> Set<collegeClassId>
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      const sizeDrafts: SizeDraft[] = product.sizes.length
        ? product.sizes.map((s) => ({
            key: uid(),
            id: s.id,
            size: s.size,
      ...
      // Rebuild classMap from product.class_mappings by matching size text
      ...
      setClassMap(map);
      setSelectedCampuses(new Set(product.campus_ids));
      ...
      setImages([]);
      const firstKey = uid();
      setSizes([{ key: firstKey, size: "", price: "", sale_price: "" }]);
      setSizesLocked(false);
      setClassMap({ [firstKey]: new Set() });
  ...
  const customerPreview = useMemo(() => {
    const parts = [
      collegeName,
      collectionType === "boys" ? "Boys" : "Girls",
      name.trim(),
      categoryName,
    ].filter((p) => p && p.trim().length > 0);
    return parts.join(" ");
  }, [collegeName, collectionType, name, categoryName]);
  ...
  const addManualSize = () => {
    const k = uid();
    setSizes((s) => [...s, { key: k, size: "", price: "", sale_price: "" }]);
    setClassMap((m) => ({ ...m, [k]: new Set() }));
  };

  const removeSize = (key: string) => {
    setSizes((prev) => {
      if (prev.length === 1 && !sizesLocked) return prev;
  ...
  const importFromTemplates = (templates: SizingTemplate[]) => {
    // Turn OFF manual size entry; append imported sizes to existing empty starter or replace all
    ...
    // Dedupe by size (case-insensitive) keeping first occurrence
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const preparedColours = colours.map((c) => {
        if (!c.colour_name.trim()) throw new Error("Colour name is required");
        if (!HEX_RE.test(c.hex_code))
          throw new Error(`Invalid hex "${c.hex_code}"`);
        return { id: c.id, colour_name: c.colour_name, hex_code: c.hex_code };
      });

      const preparedClassMappings = sizes.map((s) => ({
        size: s.size.trim(),
        class_ids: Array.from(classMap[s.key] ?? []),
      }));
      ...
      const payload = {
        ...
        class_mappings: preparedClassMappings,
      };
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["college_products", categoryId] });
      qc.invalidateQueries({ queryKey: ["categories"] });
      qc.invalidateQueries({ queryKey: ["college-products"] });
      onOpenChange(false);
    },
    ...
    <Dialog open={open} onOpenChange={(o) => !mutation.isPending && onOpenChange(o)}>
      <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Section title="Basic Info">
            <div>
              ...
              />
              <div className="mt-2 rounded-lg border border-dashed border-[#E5E7EB] bg-[#F9FAFB] px-3 py-2">
                <p className="text-[10px] uppercase tracking-[0.18em] text-black/45 font-medium">
                  Customer Sees
                </p>
                <p className="mt-0.5 text-sm font-semibold text-black break-words">
                  {customerPreview || <span className="text-black/35 font-normal">Enter a product name…</span>}
                </p>
              </div>
            </div>
            <div className="mt-3">
        ...
        >
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onSizeDragEnd}>
        ...
        <Section title="Classes (Optional)">
          {classes.length === 0 ? (
            <p className="text-xs text-black/50">
              No classes configured for this college. Add classes in the College Structure section.
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Section title="Available at Campuses">
            {campuses.length === 0 ? (
              <p className="text-xs text-black/50">No campuses configured for this college.</p>
            ) : (
              <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
                {campuses.map((c) => {
                  const checked = selectedCampuses.has(c.id);
                  const label = [c.campus_name, c.area, c.city].filter(Boolean).join(" · ");
                  return (
                    <button
                      key={c.id}
                      type="button"
                      ...
                      className={`w-full flex items-center gap-2 text-left rounded-lg px-3 py-2 text-sm transition-colors border ${
                        checked
                          ? "border-[#CF0A0A] bg-[#CF0A0A]/[0.05] text-black"
                          : "border-[#E5E7EB] hover:bg-black/[0.03] text-black/80"
                      }`}
                    >
                      <span
                        className={`h-4 w-4 rounded grid place-items-center border ${
                          checked ? "border-[#CF0A0A] bg-[#CF0A0A] text-white" : "border-black/30"
                        }`}
                      >
                        {checked && <Check className="h-3 w-3" />}
                      </span>
          ...
          <Section title="Quality Tags">
            <div className="flex gap-2">
              <Input
                ...
                placeholder="e.g. PREMIUM, DURABLE"
                className="h-10 rounded-xl"
              />
              <Button type="button" onClick={addTag} className="rounded-xl bg-black hover:bg-black/85 text-white">
                Add
              </Button>
            </div>
            {tags.length > 0 && (
                  ...
                  >
                    <span
                      className={`h-9 w-9 rounded-md border ${
                        valid ? "border-[#E5E7EB]" : "border-[#CF0A0A]"
                      }`}
        ...
        <DialogFooter className="mt-2">
          ...
          <Button
            ...
            className="rounded-lg bg-[#CF0A0A] hover:bg-[#DC5F00] text-white"
          >
            {disableSubmit ? "Saving…" : isEdit ? "Save Changes" : "Create Product"}
          </Button>
        </DialogFooter>
