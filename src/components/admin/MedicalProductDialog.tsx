                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {dup && <span className="ml-2 text-[11px] text-black/45">already added</span>}
                          </button>
                          <button
                            type="button"
...
export function MedicalProductDialog({
  open,
  onOpenChange,
  product,
}: {
  open: boolean;
  onOpenChange: (o: boolean) => void;
  product: MedicalProductDetail | null;
}) {
  const qc = useQueryClient();
  const createFn = useServerFn(createMedicalProduct);
  const updateFn = useServerFn(updateMedicalProduct);
  ...
  const [sizes, setSizes] = useState<SizeDraft[]>([{ key: uid(), size: "", price: "", sale_price: "" }]);
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setShowOnHomepage(product.show_on_homepage ?? false);
      setImages(
  ...
  const addManualSize = () =>
    setSizes((s) => [...s, { key: uid(), size: "", price: "", sale_price: "" }]);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const preparedColours = colours.map((c) => {
        ...
        if (!HEX_RE.test(c.hex_code)) throw new Error(`Invalid hex "${c.hex_code}"`);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !mutation.isPending && onOpenChange(o)}>
      <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto rounded-2xl">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Edit Medical Product" : "Add Medical Product"}</DialogTitle>
        </DialogHeader>
          ...
          <Section title="Basic Info">
            <div>
              ...
              <Input
                ...
                placeholder="e.g. Lab Coat — Long Sleeve"
            ...
            <div className="mt-3 flex items-center justify-between rounded-xl border border-[#E5E7EB] px-4 py-3">
              <div className="pr-3">
                <p className="text-sm font-medium text-black">Show on Home Page</p>
                <p className="text-xs text-black/50 leading-relaxed">
                  When enabled, this product will appear in the Home Page "Shop by Category" section under Medical.
                </p>
              </div>
        ...
        <Section title="Gender (Optional)">
          ...
          <p className="text-[11px] text-black/50 mt-2">
            Optional. Leave empty for no restriction — no gender badge is shown on frontend.
          </p>
        </Section>
        ...
        <Section title="Quality Tags">
          <div className="flex gap-2">
            <Input
              ...
              placeholder="e.g. PREMIUM, COTTON, SOFT FABRIC"
              className="h-10 rounded-xl"
            />
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setColours((prev) =>
                          prev.map((x) => (x.key === c.key ? { ...x, hex_code: v, hex_input: v } : x)),
                        );
                      }}
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setColours((prev) =>
                          prev.map((x) => (x.key === c.key ? { ...x, colour_name: e.target.value } : x)),
                        )
                      }
                    ...
                    <button
                      type="button"
                      onClick={() => setColours((prev) => prev.filter((x) => x.key !== c.key))}
